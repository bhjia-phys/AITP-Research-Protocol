...
DEFAULT_KERNEL_ROOT = Path(
    os.environ.get("AITP_KERNEL_ROOT", "/home/bhj/OpenClaw-Workspaces/research/research/knowledge-hub")
).expanduser()
...
class AITPService:
    ...
    def __post_init__(self) -> None:
        self.kernel_root = self.kernel_root.expanduser().resolve()
...
1. Start substantial topic work with `aitp bootstrap ...`, `aitp resume ...`, or `aitp loop ...`.
2. Read the generated `agent_brief.md`, `operator_console.md`, and `conformance_report.md`.
3. Register reusable operations with `aitp operation-init ...`.
...
2. inspect the generated runtime artifacts
3. register reusable operations with `aitp operation-init`
...
3. Read the generated `agent_brief.md`, `operator_console.md`, and `conformance_report.md`.
4. Prefer `aitp loop ...` whenever auto-run capability/trust checks are relevant.
5. Continue the task only after the runtime artifacts exist.
"""
...
Then inspect `loop_state.json`, `capability_report.md`, and `conformance_report.md`.
"""
        else:
            body = """---
...
"""
    ...
    def install_agent(
        ...
        *,
        ...
        targets = [agent] if agent != "all" else ["codex", "openclaw", "opencode"]
