...
read surfaces, and the new Gate 0 operations. The current shared worktree also
...
- Capability registry, Gate 0 wrapper behavior, and optional-extension
...
- Architecture baseline: 2 failed and 4 passed, with the same 39 pre-existing
  oversized modules and the existing 1,509-line `cli.py` boundary failure.

The 9-minute adapter matrix must become a separate slow CI lane in Task 9;
the capability parity lane remains the fast feedback path. New Task 8 modules
are at most 320 lines and add no architecture-boundary violation.
