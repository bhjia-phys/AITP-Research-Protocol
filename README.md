...
For validation, promotion-packet creation, and L2 promotion MCP calls, Claude
`PreToolUse` resolves the active typed claim plus evidence/code-state refs and
reuses kernel policy before the tool runs. Other adapters can call the same
contract through
...
promotion-packet creation, when their requested source is only a
...
validation-contract, promotion-packet, validation, and promotion gate
sequences, so runtimes can see that policy evaluation comes before typed
record creation, preflight, or promotion. The small
