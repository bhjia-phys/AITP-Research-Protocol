...
def preview_quiet_checkpoint_batch(
    ...
    *,
    ...
    existing_refs = _existing_closeout_record_refs(
        ws,
...
def apply_quiet_checkpoint_batch(
    ...
    *,
    ...
    audit = build_record_completeness_audit(
        ...
        source_specs=_dict_list(source_specs),
        tool_run_specs=_dict_list(tool_run_specs),
        written_refs=_dedupe(
            written_refs
            + _existing_closeout_record_refs(
                ws,
                topic_id=topic_id,
                claim_id=focus_claim_id,
                run_id=run_id,
...
def _existing_closeout_record_refs(
    ...
    *,
    ...
    for recipe in list_valid_records(ws.registry_dir("tool_recipes"), ToolRecipeRecord):
        if recipe.recipe_id in matching_recipe_ids:
            refs.append(f"tool_recipe:{recipe.recipe_id}")

    for result in list_valid_records(ws.registry_dir("validation_results"), ValidationResultRecord):
    ...
    for asset in list_valid_records(ws.registry_dir("source_assets"), SourceAssetRecord):
    ...
    for report in list_valid_records(ws.registry_dir("sensemaking_reports"), SensemakingReportRecord):
