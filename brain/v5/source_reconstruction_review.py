...
def _legacy_records(directory, cls):
    return _list_valid_records(
        directory,
        cls,
        operation="legacy_source_reconstruction_review",
    )
...
def _validate_typed_basis_refs(
    ...
    *,
    ...
    _require_same_claim_refs(
        ...
        _legacy_records(ws.registry_dir("reference_locations"), ReferenceLocationRecord),
        "location_id",
        claim_id,
    )
    _require_topic_refs(
        "physics object",
        object_ids,
        _legacy_records(ws.registry_dir("physics_objects"), PhysicsObjectRecord),
        "object_id",
        topic_id,
    )
    _require_relation_refs(
        relation_ids,
        _legacy_records(ws.registry_dir("object_relations"), ObjectRelationRecord),
        topic_id,
        claim_id,
    )
