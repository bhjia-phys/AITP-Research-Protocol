...
def _validate_mcp_arguments(target: dict[str, Any], path: str, result: ContractResult) -> None:
    ...
    if entrypoint_key in {
        ...
        if entrypoint_key == "literature_extraction_report":
            if arguments.get("required") != ["base", "session_id", "source_refs"]:
                result.add(
                    f"{path}.mcp_arguments.required",
                    "must require base, session_id, and source_refs",
                )
            if arguments.get("optional") != ["report_profile", "focus_terms", "optional_claim_id"]:
                result.add(
                    f"{path}.mcp_arguments.optional",
        ...
        if entrypoint_key == "literature_source_set_readiness":
            ...
            if arguments.get("optional") != ["optional_claim_id", "readiness_scope"]:
                result.add(
                    f"{path}.mcp_arguments.optional",
                    "must allow optional_claim_id and readiness_scope",
                )
        if entrypoint_key == "context_profile_templates":
            if arguments.get("required") != []:
                result.add(f"{path}.mcp_arguments.required", "must be empty")
