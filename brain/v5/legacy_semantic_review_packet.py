...
def _legacy_records(directory, cls):
    return _list_valid_records(
        directory,
        cls,
        operation="legacy_semantic_review_packet",
    )
...
def _claim_payload(ws: WorkspacePaths, claim_id: str) -> dict[str, Any]:
    if not claim_id:
        return {}
    claims = {claim.claim_id: claim for claim in _legacy_records(ws.registry_dir("claims"), ClaimRecord)}
...
def _typed_records_for_claim(ws: WorkspacePaths, topic: str, claim_id: str) -> dict[str, list[dict[str, Any]]]:
    return {
        ...
        "sensemaking_reports": [
            ...
            for record in _legacy_records(ws.registry_dir("sensemaking_reports"), SensemakingReportRecord)
            if record.claim_id == claim_id
        ],
        "validation_results": [
