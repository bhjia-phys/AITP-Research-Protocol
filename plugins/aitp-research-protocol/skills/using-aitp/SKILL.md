...
After successful configuration, tell the user to restart the MCP server or open a new Codex thread so the full `aitp_v5_*` tool surface loads. If the user does not have a checkout, offer to clone `https://github.com/bhjia-phys/AITP-Research-Protocol.git` into a user-chosen directory before calling `aitp_configure`.
...
4. For known sessions, call `aitp_v5_get_execution_brief(base="", session_id=<session-id>)`, then `aitp_v5_get_claim_relation_map` for the same session.
5. If only a topic slug is known, call `aitp_v5_build_workspace_recovery_audit(base="", topics=[<topic>])`. Use a `recovery_ready` row's session and claim before creating or migrating anything.
6. If no usable v5 session exists, use v5 migration or topic/claim/session creation tools. Do not write progress into old L0/L1/L3/L4 files.
7. Load `aitp-runtime` before active continuation, derivation, validation, numerical work, final synthesis, trust updates, or L2 promotion.

## Intensity Policy

- Status or prior-progress inquiry: read-only recovery, brief, relation map, and concise summary. Do not write unless the user asks for a durable handoff or resolves a human checkpoint.
