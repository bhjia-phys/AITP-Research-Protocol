#!/usr/bin/env python3
"""Launch AITP v5 MCP for the Kimi Code plugin, or a first-run setup MCP when configuration is missing."""
...
CONFIG_PATH = Path.home() / ".aitp" / "kimi-plugin-config.json"
...
class SetupMcpServer:
    ...
    def aitp_config_status(self) -> dict:
        """Show whether the AITP Kimi Code plugin is configured."""
    ...
    def aitp_configure(self, repo_root: str, topics_root: str = "") -> dict:
        """Persist the AITP repo path and topics root for this Kimi Code plugin."""
